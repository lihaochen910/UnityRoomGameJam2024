...
namespace Bang.Unity.Serialization
{
    ...
    public class SerializedMethodInfo : ISerializedMethodBaseInfo
    {
        ...
        void ISerializationCallbackReceiver.OnAfterDeserialize() {
            ...
            var type = ReflectionHelper.GetType(split[0], true);
            if ( type == null ) {
                _method = null;
                return;
            }
            ...
            var name = split[1];
            var returnType = split.Length >= 3 ? ReflectionHelper.GetType(split[2], true) : null;
            ...
            for ( var i = 0; i < parameterTypes.Length; i++ ) {
                var pType = ReflectionHelper.GetType(paramTypeNames[i], true);
                if ( pType == null ) {
                    paramsFail = true;
                    break;
                }
            ...
            if ( !paramsFail ) {
                ...
                if ( isSerializedGeneric ) {
                    ...
                    for ( var i = 0; i < genericArgTypes.Length; i++ ) {
                        var argType = ReflectionHelper.GetType(genericArgTypeNames[i], true);
                        if ( argType == null ) {
            ...
            if ( _method == null ) {
                ...
                if ( _method != null && _method.IsGenericMethod ) {
                    var argType = isSerializedGeneric ? ReflectionHelper.GetType(_genericArgumentsInfo.Split('|').First(), true) : _method.GetFirstGenericParameterConstraintType();
